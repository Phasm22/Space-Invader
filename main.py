...
from ctypes.wintypes import HDESK, HENHMETAFILE
...
GREEN_SPACE_SHIP = pygame.image.load(
    os.path.join("assests", "pixel_ship_green_small.png")
)
BLUE_SPACE_SHIP = pygame.image.load(
    os.path.join("assests", "pixel_ship_blue_small.png")
)
# Player Ship
YELLOW_SPACE_SHIP = pygame.image.load(
    os.path.join("assests", "pixel_ship_small.png")
).convert_alpha()

YELLOW_SPACE_SHIP = pygame.transform.scale(YELLOW_SPACE_SHIP, (75, 75))
...
def main():
    ...
    while run:
        ...
        if (
            keys[pygame.K_a] or keys[pygame.K_LEFT] and player.x - player_velo > 0
        ):  # left
            player.x -= player_velo
        if (
            keys[pygame.K_d]
            or keys[pygame.K_RIGHT]
            and player.x + player_velo + 50 < WIDTH
        ):  # right
            player.x += player_velo
        if keys[pygame.K_w] or keys[pygame.K_UP] and player.y - player_velo > 0:  # up
            player.y -= player_velo
        if (
            keys[pygame.K_s]
            or keys[pygame.K_DOWN]
            and player.y + player_velo + 50 < HEIGHT
        ):  # down
            player.y += player_velo
